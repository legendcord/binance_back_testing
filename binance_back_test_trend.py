...
data_hour = pd.read_csv(hour_str+'_data/' + symbol + '/ETHUSDT.csv')
...
def get_current_pivot_points(recent_kline, left_bars, right_bars):
    ...
    for index, high_val in enumerate(past_highs):
        ...
        if index >= right_bars and index <= (bar_count-1-left_bars):
            # only need to look at neighboring points; note python includes left boundary and exludes right boundary when indexing list
...
def update_cumulative_return(orderDirection, pv_enum_str):
    global last_short_prices, last_long_prices, previous_order_sizes, cumulative_return, portfolio_value, max_port, long_orders, short_orders
    ...
    if orderDirection == 1:
        ...
        short_price_sum = 0
        for i in short_orders[pv_enum_str]:
            short_price_sum += i['price']
        avg_short_price = short_price_sum / len(short_orders[pv_enum_str])
        total_order_size = previous_order_size * len(short_orders[pv_enum_str])
        print('\nSold at ' + str(avg_short_price) + ' and Bought at ' + str(last_long_price) + ' with size ' + str(total_order_size))
        cumulative_return *= 1 + (avg_short_price - last_long_price) / avg_short_price * total_order_size / portfolio_value
    ...
    if orderDirection == -1:
        ...
        long_price_sum = 0
        for i in long_orders[pv_enum_str]:
            long_price_sum += i['price']
        avg_long_price = long_price_sum / len(long_orders[pv_enum_str])
        total_order_size = previous_order_size * len(long_orders[pv_enum_str])
        print('\nBought at ' + str(avg_long_price) + ' and Sold at ' + str(last_short_price) + ' with size ' + str(total_order_size))
        cumulative_return *= 1 + (last_short_price - avg_long_price) / avg_long_price * total_order_size / portfolio_value
...
def place_order_based_on_state(pv_enum_str, last_low, last_high, dt_str, current_close):
    global base_curve, base_price, pv_status, pv_highs, pv_lows, previous_order_sizes, last_short_prices, last_long_prices, long_orders, short_orders, max_orders
    ...
    if order_status == 0:        
        ...
        if last_low < pvl:
            ...
            print('zero ' + pv_enum_str + ' status => ' + '\033[1m' + 'go short' + '\033[0m')
            ...
            long_orders[pv_enum_str] = []
            short_orders[pv_enum_str] = [{'price':pvl, 'size':order_size}]
        elif last_high > pvh:
            print('\nCurrent date is ' + dt_str)
            print('zero ' + pv_enum_str + ' status => ' + '\033[1m' + 'go long' + '\033[0m')
            ...
            #update completed orders
            short_orders[pv_enum_str] = []
            long_orders[pv_enum_str] = [{'price':pvh, 'size':order_size}]
    # case 2 - positive position
        ...
        if last_low < pvl:
            ...
            # following need to happen after update_cumulative_return
            previous_order_sizes[pv_enum_str] = order_size
            #update completed orders
            long_orders[pv_enum_str] = []
            short_orders[pv_enum_str] = [{'price':pvl, 'size':order_size}]
        elif last_high > pvh and len(long_orders[pv_enum_str]) < max_orders and pvl > long_orders[pv_enum_str][-1]['price']: # add more long position if current low is higher than last long
            print('\nCurrent date is ' + dt_str)
            print('positive ' + pv_enum_str + ' position => add more long position')
            print('last_high is ' + str(last_high))
            print('current ' + pv_enum_str + ' pivot high is ' + str(pvh))
            order_size = long_orders[pv_enum_str][-1]['size']
            long_orders[pv_enum_str].append({'price':pvh, 'size':order_size});
    # case 3 - negative position
        ...
        if last_high > pvh:
            ...
            short_orders[pv_enum_str] = []
            long_orders[pv_enum_str] = [{'price':pvh, 'size':order_size}]
        elif last_low < pvl and len(short_orders[pv_enum_str]) < max_orders and pvh < short_orders[pv_enum_str][-1]['price']: # add more short position if current high is lower than last short
            print('\nCurrent date is ' + dt_str)
            print('negative ' + pv_enum_str + ' position => add more short position')
            print('last_low is ' + str(last_low))
            print('current ' + pv_enum_str + ' pivot low is ' + str(pvl))
            order_size = short_orders[pv_enum_str][-1]['size']
            short_orders[pv_enum_str].append({'price':pvl, 'size':order_size});            
