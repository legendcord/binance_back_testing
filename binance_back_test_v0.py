...
def get_current_pivot_points(recent_kline_4h, left_bars, right_bars):
    ...
    for i in range(bar_count):
        ...
        past_highs.append(recent_kline_4h[i][2]) # high
        past_lows.append(recent_kline_4h[i][3]) # low
...
def update_cumulative_return(orderDirection, pv_enum_str):
    ...
    last_short_price = last_short_prices[pv_enum_str] * 0.998 # friction
    last_long_price = last_long_prices[pv_enum_str] * 1.002 # friction
...
for i in range(rows):
    ...
    if dt_obj.hour % 4 == 0 and dt_obj.minute == 0 and dt_obj.second == 0:
        dt_obj_4h = get_datetime(data_4h[j][0])
        while dt_obj_4h < dt_obj:
        ...
        for index in range(len(pv_enum)):
            left_bars = int(pv_enum[index][1])
            right_bars = int(pv_enum[index][3])
