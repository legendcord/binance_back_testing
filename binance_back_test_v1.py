...
#pv_enum = ['l6r1', 'l4r2', 'l2r6', 'l4r6']
#pv_enum = ['l6r1', 'l4r2', 'l2r6', 'l4r6', 'l4r4']
...
use_stop_loss = False # use L2R2 points as the exit price
sl_left_bar = 2
sl_right_bar = 2
to_short_USL = True # _USL use stop loss
to_long_USL = True # _USL use stop loss
...
long_order_exit_price = None
short_order_exit_price = None
PVH_updated = False
PVL_updated = False
pv_highs = {}
pv_lows = {}
...
def get_and_update_pivot_points(recent_kline, left_bars, right_bars):
    global PVH_updated, PVL_updated, current_pivot_high, current_pivot_low, pv_highs, pv_lows, long_order_exit_price, short_order_exit_price
    
    get_current_pivot_points(recent_kline, left_bars, right_bars)
    if left_bars == sl_left_bar and right_bars == sl_right_bar:
        if PVL_updated:
            long_order_exit_price = current_pivot_low
        if PVH_updated:
            short_order_exit_price = current_pivot_high            
    else:
...
def update_cumulative_return(orderDirection, pv_enum_str):
    global last_short_prices, last_long_prices, previous_order_sizes, cumulative_return, portfolio_value, max_port, use_stop_loss, short_order_exit_price, long_order_exit_price
    ...
    if use_stop_loss:
        if orderDirection == 1 and last_short_price is None:
            raise ValueError('last_short_price should be valid')
        if orderDirection == -1 and last_long_price is None:
            raise ValueError('last_long_price should be valid')                
    elif last_short_price is None or last_long_price is None:
    ...
    if orderDirection == 1:
        if use_stop_loss:
            #print('\nSold at ' + str(last_short_price) + ' and Bought at ' + str(short_order_exit_price))
            cumulative_return *= 1 + (last_short_price - short_order_exit_price) / last_short_price * previous_order_size / portfolio_value
        else:
    ...
    if orderDirection == -1:
        if use_stop_loss:
            #print('\nBought at ' + str(last_long_price) + ' and Sold at ' + str(long_order_exit_price))
            cumulative_return *= 1 + (long_order_exit_price - last_long_price) / last_long_price * previous_order_size / portfolio_value
        else:
...
def place_order_based_on_state(pv_enum_str, last_low, last_high, dt_str, current_close):
    global base_curve, base_price, pv_status, pv_highs, pv_lows, previous_order_sizes, last_short_prices, last_long_prices, use_stop_loss, long_order_exit_price, short_order_exit_price, to_short_USL, to_long_USL
    ...
    if order_status == 0:        
        ...
        if use_stop_loss:
            if to_short_USL and last_low < pvl:
                to_short_USL = False
                to_long_USL = True
            ...
            if to_long_USL and last_high > pvh:
                to_short_USL = True
                to_long_USL = False
                print('\nCurrent date is ' + dt_str)
    ...
    # case 2 - positive position
        ...
        if use_stop_loss and last_low < long_order_exit_price:
            pv_status[pv_enum_str] = 0
    ...
    # case 3 - negative position
        ...
        if use_stop_loss and last_high > short_order_exit_price:
            pv_status[pv_enum_str] = 0
            update_cumulative_return(1, pv_enum_str)
            base_curve.append(current_close/base_price)
            # return here
...
for i in range(rows):
    ...
    if dt_obj.hour % hour_num == 0 and dt_obj.minute == 0 and dt_obj.second == 0:
        ...
        if use_stop_loss:
            # update pivot point as exit order price
            get_and_update_pivot_points(kline_hour[-num_points:], sl_left_bar, sl_right_bar)
